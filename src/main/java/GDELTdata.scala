...
/**
  * Created by lws4 on 14.07.2016.
  */
class GDELTdata(sqlContext: SQLContext,input:String) {
  ...
  def readReferenceEventCode(refEventCode: String, country: String, output: String): DataFrame = {
    val gdelt_reduced = sqlContextData
      .select("GLOBALEVENTID", "EventCode", "ActionGeo_CountryCode", "ActionGeo_Lat", "ActionGeo_Long")
      .where("ActionGeo_Lat is not null")
      .where("ActionGeo_Lat is not null")
